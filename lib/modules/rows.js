module.exports = function(h) {
  return (classes) => {
    ...
    if (this.source === 'client') {
      ...
      if (!data.length && this.source === 'client' && this.page !== 1) {
    ...
    data.map((row, index) => {
      if (
        this.opts.groupBy &&
        this.source === 'client' &&
        row[this.opts.groupBy] !== currentGroup
      ) {
        groupSlot = this.getGroupSlot(row[this.opts.groupBy])
        groupValue = row[this.opts.groupBy]

        groupByContent = this.opts.toggleGroups ? (
          <button class={classes.button} on-click={this.toggleGroup.bind(this, groupValue)}>
            {groupValue}
            <span class={this.groupToggleIcon(groupValue)} />
          </button>
        ) : (
          groupValue
        )
        ...
        rows.push(
          <tr class={classes.groupTr} on-click={this._toggleGroupDirection.bind(this)}>
      ...
      if (this.hasChildRow) {
        var childRowToggler = (
          <td>
            <span
              on-click={this.toggleChildRow.bind(this, row[rowKey])}
      ...
      rows.push(
        <tr
          class={rowClass}
          on-click={this.rowWasClicked.bind(this, row)}
          on-dblclick={this.rowWasClicked.bind(this, row)}
          key={this.opts.draggableRows ? `row-${row[this.opts.draggableIdentity]}` : undefined } // may be useful when dragging
        >
          {columns}{' '}
        </tr>
      )
      ...
      rows.push(
        this.hasChildRow && this.openChildRows.includes(row[rowKey]) ? (
          <tr
            class="VueTables__child-row"
            key={this.opts.draggableRows ? `child-row-${row[this.opts.draggableIdentity]}` : undefined } // may be useful when dragging
          >
