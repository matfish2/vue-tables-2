  ...
  return (classes) => {
    ...
    data.map((row, index) => {
      ...
      if (this.hasChildRow) {
        var childRowToggler =
          <td>
            { typeof row.show_toggler !== 'undefined' && row.show_toggler === false
              ? ''
              : <span on-click={this.toggleChildRow.bind(this,row[rowKey])} class={`VueTables__child-row-toggler ` + this.childRowTogglerClass(row[rowKey])}></span>
            }
          </td>
      ...
      if (this.hasChildRow && this.openChildRows.includes(row[rowKey])) {
        rows.push(
          this.opts.childRowTr
            ? <tr class='VueTables__child-row'>
                {this.opts.childRowTd
