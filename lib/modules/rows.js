  ...
  return (classes) => {
    ...
    data.map((row, index) => {
      ...
      this.allColumns.map(column => {
        ...
        revertValue = revertVal(row, column).bind(this);

        columns.push(<td class={`${this.columnClass(column)} ${this._cellClasses(column, row)}`.trim()} tabindex="0">
        {rowTemplate ? rowTemplate({ row, column, index, update, isEditing, setEditing, revertValue }) : this.render(row, column, index, h)}
        </td>)
      });
      ...
      rows.push(<tr {...{attrs: rowAttributes}} class={rowClass} on-click={this.rowWasClicked.bind(this, row, index)} on-dblclick={this.rowWasClicked.bind(this, row, index)}>{columns} </tr>);
