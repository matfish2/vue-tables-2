...
module.exports = function(h) {
  ...
  return (classes) => {
    ...
    data.map((row, index) => {
      ...
      rows.push(<tr {...{attrs: rowAttributes}} class={`VueTables__row ${rowClass}`} on-click={this.rowWasClicked.bind(this, row, index)} on-dblclick={this.rowWasClicked.bind(this, row, index)}>{columns} </tr>);

      rows.push(this.hasChildRow && this.openChildRows.includes(row[rowKey])?

      <tr class={`VueTables__child-row ${rowClass}`}><td colspan={this.colspan} tabindex="0">{this._getChildRowTemplate(h, row)}</td></tr>:h());
