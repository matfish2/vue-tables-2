...
module.exports = function(h, modules, classes, slots) {
  ...
  var tableTop = <div class={classes.row} v-show={shouldShowTop}>
    ...
    var closeAllChildRows = () => {
      var chrs = [...this.openChildRows];
      chrs.forEach(row => {
        if (row) this.toggleChildRow(row)
      })
    }
    ...
    {!this.opts.draggableRows ? (
      ...
      <draggable v-model="data" onStart={closeAllChildRows} onEnd={(e) => this.opts.draggableCalback(e)} class="VueTables__draggable-rows" tag="tbody" draggable="tr:not(.VueTables__child-row):not(.VueTables__no-results)">
        {slots.prependBody}
        {modules.rows(classes)}
        {slots.appendBody}
      </draggable>
