...
module.exports = function(h, modules, classes, slots) {
  ...
  var tableTop = <div class={classes.row} v-show={shouldShowTop}>
    ...
    {!this.opts.draggableRows ? (
      ...
      <draggable v-model="data" onEnd={(e) => {this.opts.draggableCalback(e)}} class="VueTables__draggable-rows" tag="tbody" draggable="tr:not(.VueTables__child-row):not(.VueTables__no-results)">
        {slots.prependBody}
        {modules.rows(classes)}
        {slots.appendBody}
      </draggable>
