...
collapseGroups (client-side) | Boolean | When using the `groupBy` option, settings this to `true` will initalize the groups in a collapsed state, i.e when the members of the groups are hidden. The group value will be contained in a button to enable toggling rows' visibility | `false`
